...
data.fs.counts <- as.matrix(read.table(paste(directory,"/raw_data/data_2dgas_fs/RawReadMatrix.txt",sep="")))

sample.information <- read.table(paste(directory,"/raw_data/data_2dgas_fs/sample_information.txt",sep=""),
                                 header = TRUE)[colnames(data.fs.counts),]

cells <- paste(sample.information$time,sample.information$condition,sep="-")

row.names(data.fs.counts) <- toupper(row.names(data.fs.counts))
colnames(data.fs.counts) <- paste(cells,"_",colnames(data.fs.counts ),sep="")

dataset.fs <- CreateSeuratObject(data.fs.counts , project = "fs", min.cells = 1)
...
dataset.fs <- subset(dataset.fs, subset = condition %in% c("WNT","BMP") & nCount_RNA > 50000 & nFeature_RNA > 5000 & percent.mito < 0.05)

# QC plots
sums <- data.frame(nb_transcript=log10(dataset.fs$nCount_RNA))
Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/QC/Histogram-transcript.svg",sep=""),type="svg",width = 4,height = 4,units="cm",dpi=20)
ggplot(sums, aes(x=nb_transcript)) + geom_histogram(binwidth=0.1)+
  geom_vline(xintercept = mean(sums$nb_transcript), color = "red", size=1.5)
dev.off()

sums <- data.frame(nb_transcript=log10(dataset.fs$nFeature_RNA))
Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/QC/Histogram-nGenes.svg",sep=""),type="svg",width = 4,height = 4,units="cm",dpi=20)
ggplot(sums, aes(x=nb_transcript)) + geom_histogram(binwidth=0.1)+
  geom_vline(xintercept = mean(sums$nb_transcript), color = "red", size=1.5)
dev.off()

for (to.plot in c("nFeature_RNA", "nCount_RNA", "percent.mito")){
  Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/QC/VLN-",to.plot,".svg",sep=""),type="svg",width = 6,height = 6,units="cm",dpi=20)
  print(VlnPlot(object = dataset.fs, features=to.plot,slot = "counts"))
  dev.off()
}

Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/QC/Scatter-CountVsFeature.svg",sep=""),type="svg",width = 6,height = 6,units="cm",dpi=20)
FeatureScatter(object = dataset.fs, feature1 = "nCount_RNA", feature2 = "nFeature_RNA",slot = "counts")
dev.off()

Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/QC/Scatter-CountVsGapdh.svg",sep=""),type="svg",width = 6,height = 6,units="cm",dpi=20)
FeatureScatter(object = dataset.fs, feature1 = "nCount_RNA", feature2 = "GAPDH",
               slot = "counts")
dev.off()

## Scale the data to have 1 000 000 counts (the mean is 85000) in each cell and then, normalizing the data with log1p
mean(dataset.fs$nCount_RNA)
dataset.fs <- NormalizeData(object = dataset.fs, verbose = FALSE,scale.factor = 1000000)
...
for (gene in c("T","EOMES","POU5F1")){
  ...
  Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/QC/Scatter-Scalling-",gene,".svg",sep=""),type="svg",width = 4,height = 4,units="cm",dpi=20)
...
dataset.fs <- FindVariableFeatures(dataset.fs, selection.method = "vst", nfeatures = 5000)
...
Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/PCA/PCA-Plot-1-2.svg",sep=""),type="svg",width = 4,height = 4,units="cm",dpi=20)
PCAPlot(dataset.fs)
dev.off()
...
Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/PCA/PCA-JackStrawPlot-raw.svg",sep=""),type="svg",width = 6,height = 4,units="cm",dpi=20)
JackStrawPlot(dataset.fs,dims = 1:30)
dev.off()

jackstraw.pval <- -log10(dataset.fs@reductions$pca@jackstraw@overall.p.values[,"Score"])
ndims <- 30
Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/PCA/PCA-JackStrawPlot-pval.svg",sep=""),type="svg",width = 4,height = 4,units="cm",dpi=20)
...
Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/PCA/PCA-ElbowPlot.svg",sep=""),type="svg",width = 4,height = 4,units="cm",dpi=20)
...
dataset.fs@reductions$umap@cell.embeddings[,2] = -dataset.fs@reductions$umap@cell.embeddings[,2]

Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/GenePlots/UMAP-Condition.svg",sep=""),type="svg",width = 5,height = 4,units="cm",dpi=20)
...
filename = paste(directory,"/figures/2dgas-fs/Counts/GenePlots/Cluster-stability-analysis.svg",sep="")
...
Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/GenePlots/UMAP-Cluster-raw.svg",sep=""),type="svg",width = 4,height = 4,units="cm",dpi=20)
...
new.cluster.ids <- c("NasMes2","PGC","NasMes1","PS","Epi","DE","aPS",
                     "AxMes","ExeMes")
new.cluster.ids.order <- c("Epi","PS","aPS","DefEnd","AxMes",
                           "NasMes1","NasMes2","ExeMes","PGC")

dataset.fs@active.ident <- plyr::mapvalues(x = dataset.fs@active.ident, from = current.cluster.ids, to = new.cluster.ids)
dataset.fs@active.ident  <- factor(dataset.fs@active.ident , levels = new.cluster.ids.order)
dataset.fs$celltype <- dataset.fs@active.ident

Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/GenePlots/UMAP-Cluster-annotated.svg",sep=""),type="svg",width = 4,height = 4,units="cm",dpi=20)
DimPlot(dataset.fs, reduction = "umap",dims=c(1,2),label=TRUE,label.size = size)+
  theme(axis.text = element_text(size=size*2),
        axis.title = element_text(size=size*2))+
  NoLegend()
dev.off()
...
# Save the dataset
saveRDS(dataset.fs,file = paste(directory,"/rds/2dgas_t36_fs_counts.rds",sep=""))

# Load the dataset from the commented lines above
dataset.fs <- readRDS(file = paste(directory,"/rds/2dgas_t36_fs_counts.rds",sep=""))
...
sce <- as.SingleCellExperiment(dataset.fs)
table(sce$ident)
dm <- DiffusionMap(sce, verbose = TRUE,n_pcs=10,k=100)
...
DoHeatmap(dataset.fs,top10.tf$gene)

# Export data to use miic
top.tf<- markers.all.tf %>% group_by(cluster) %>% top_n(n = 50, wt = avg_log2FC)
other.tf <- c("ID3","LEF1","POU5F1","PRDM1","PRDM14","T",
              "SMAD1","SMAD2","SMAD3","SMAD4","SMAD5","SMAD6","SMAD7","SMAD9")

list.gene.use <- sort(union(unique(top.tf$gene),other.tf))

filename <- paste(directory,"/raw_data/data_2dgas_fs/csv/2Dgas-t36-fs_top50-cluster-TF_v1.csv",sep="")
matrix.use <- as.data.frame(t(dataset.fs@assays$RNA@counts[list.gene.use,]))
write.csv(matrix.use,filename,row.names = FALSE)

filename <- paste(directory,"/raw_data/data_2dgas_fs/csv/2Dgas-t36-fs_top50-cluster-TF_v2.csv",sep="")
matrix.use$condition <- as.numeric(factor(dataset.fs@meta.data$condition))
write.csv(matrix.use,filename,row.names = FALSE)

filename <- paste(directory,"/raw_data/data_2dgas_fs/csv/2Dgas-t36-fs_top50-cluster-TF_norm_v1.csv",sep="")
matrix.use <- as.data.frame(round(t(dataset.fs@assays$RNA@data[list.gene.use,]),1))
write.csv(matrix.use,filename,row.names = FALSE)
...
gene.list.signalling <- sort(intersect(toupper(GO.genes.signalling ),row.names(dataset.fs[['RNA']]@counts)))

markers.all.signalling <- FindAllMarkers(dataset.fs,only.pos = TRUE, features = gene.list.signalling, min.diff.pct = 0.1)
top10.signalling <- markers.all.signalling %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)

DoHeatmap(dataset.fs,top10.signalling$gene)
...
gene.heatmap <- c("SOX2","POU3F1","UTF1","CDH1","CLDN6",
                  ...
                  "FOXF1","HAND1","BMP4","KDR","ETV2",
                  "DND1","PRDM1","PRDM14","KLF5","CDX2")
DoHeatmap(dataset.fs,gene.heatmap)

g <- DotPlot(dataset.fs,features=gene.heatmap)
for (i in 1:8){
  g <- g + geom_vline(xintercept=5*i+0.5)
}
filename = paste(directory,"/figures/2dgas-fs/Counts/GenePlots/dotplot_list-genes.svg",sep="")
...
for (gene in genes.to.plot){
  Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/GenePlots/UMAP-",gene,".svg",sep=""),type="svg",width = 4,height = 4,units="cm",dpi=20)
  print(FeaturePlot(dataset.fs,gene,pt.size = 0.5,label=FALSE,order = TRUE,label.size = 5)+
          theme(axis.ticks.x = element_blank(),axis.text.x = element_blank(),axis.title.x=element_blank(),
                axis.ticks.y = element_blank(),axis.text.y = element_blank(),axis.title.y=element_blank(),
                axis.line=element_blank()))
  dev.off()
  Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/GenePlots/VLN-",gene,".svg",sep=""),type="svg",width = 4,height = 4,units="cm",dpi=20)
  print(VlnPlot(dataset.fs,gene,pt.size=0.1)+NoLegend())
  dev.off()
}

list.conditions <- c("t35-BMP","t36-BMP","t37-BMP","t35-WNT","t36-WNT","t37-WNT")
for (celltype.i in new.cluster.ids.order){
  dataset.cells <- subset(dataset.fs, subset = celltype ==celltype.i)
  ...
  Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/BarplotCondition/BarPlot-Condition-",celltype.i,".jpg",sep=""),type="svg",width = 5,height = 5,units="cm",dpi=20)
...
dataset.fs$celltype.condition <- paste(gsub('[0-9]+', '', dataset.fs$celltype),dataset.fs$condition,sep=".")
...
dataset.fs$celltype.condition  <- factor(dataset.fs$celltype.condition , levels = celltype.condition.order)

for (gene in genes.to.plot){
  Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/GenePlots/VLN2-",gene,".svg",sep=""),type="svg",width = 5,height = 5,units="cm",dpi=20)
  print(VlnPlot(dataset.fs,gene,group.by = "celltype.condition",pt.size=0)+
...
for (time.emb in list.time.emb){
  ...
  intersect.genes <- intersect(row.names(dataset.emb),row.names(dataset.fs))
  ...
  intersect_genes <- intersect(row.names(dataset.emb[['RNA']]),row.names(dataset.fs[['RNA']]))
  # union_var <- union(VariableFeatures(dataset.fs),VariableFeatures(dataset.emb))
  ...
  dataset.fs[['RNA']]@counts <- dataset.fs[['RNA']]@counts[intersect_genes,]
  dataset.emb[['RNA']]@data<- dataset.emb[['RNA']]@data[intersect_genes,]
  dataset.fs[['RNA']]@data <- dataset.fs[['RNA']]@data[intersect_genes,]
  dataset.emb[['RNA']]@scale.data<- dataset.emb[['RNA']]@scale.data[intersect_genes,]
  dataset.fs[['RNA']]@scale.data <- dataset.fs[['RNA']]@scale.data[intersect_genes,]
  
  dataset.mnn <- RunFastMNN(object.list = c(dataset.emb,dataset.fs),features=feature,
                            d=nb.pc, k = k.anchor)
  ...
  dataset.mnn$celltype.emb.only[dataset.mnn$celltype %in% unique(dataset.fs$celltype)] <- time.gas
  ...
  list.celltype.gas <- rev(sort(unique(Idents(dataset.fs))))
  ...
  for (i in 1:n.celltype.emb){
    ...
    for (j in 1:n.celltype.gas){
      ...
      dataset.fs.mnn <- subset(dataset.mnn,subset = celltype == celltype.gas)
      
      df.cor.gas <- cor(t(dataset.emb.mnn@reductions$mnn@cell.embeddings),t(dataset.fs.mnn@reductions$mnn@cell.embeddings))
      cor.gas <- mean(apply(df.cor.gas,2,mean))
      matrix.cor[i,j] <- cor.gas # /cor.emb
      
      df.dist.gas <- as.matrix(pdist(dataset.emb.mnn@reductions$mnn@cell.embeddings,dataset.fs.mnn@reductions$mnn@cell.embeddings))
  ...
  Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/Comparison/",time.gas,"vs",time.emb,"_mnn_heatmap_pearson-correlation.svg",sep="")
  ...
  Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/Comparison/",time.gas,"vs",time.emb,"_mnn_heatmap_euclidean-distance.svg",sep="")
        ,type="svg",width = 3+n.celltype.emb,height = 2+n.celltype.gas,units="cm",dpi=20)
  print(ggplot(data = matrix.melted, aes(x=Var1, y=Var2, fill=value)) + 
          geom_tile(color = "white")+
...
dataset.fs <- readRDS(file = paste(directory,"/rds/2dgas_t36_ss3.rds",sep=""))

dataset.fs$dataset <- time.fs
dataset.fs$dataset <- time.ss3

intersect.genes <- intersect(row.names(dataset.fs),row.names(dataset.fs))
...
intersect_genes <- intersect(row.names(dataset.fs[['RNA']]),row.names(dataset.fs[['RNA']]))
# union_var <- union(VariableFeatures(dataset.fs),VariableFeatures(dataset.fs))
union_var <- union(VariableFeatures(dataset.fs),VariableFeatures(dataset.fs))
feature <- intersect(intersect_genes,union_var)
dataset.fs[['RNA']]@counts <- dataset.fs[['RNA']]@counts[intersect_genes,]
dataset.fs[['RNA']]@counts <- dataset.fs[['RNA']]@counts[intersect_genes,]
...
dataset.mnn <- RunFastMNN(object.list = c(dataset.fs,dataset.fs),features=feature,
                          d=nb.pc, k = k.anchor)
...
dataset.mnn$celltype.fs.only <- dataset.mnn$celltype
dataset.mnn$celltype.ss3.only[(dataset.mnn$celltype %in% unique(dataset.fs$celltype))&
                                (dataset.mnn$dataset == time.fs)] <- time.fs
dataset.mnn$celltype.fs.only[(dataset.mnn$celltype %in% unique(dataset.fs$celltype))&
...
## Correlation with MNN reduction
list.celltype.ss3 <- sort(unique(Idents(dataset.fs)))
list.celltype.fs <- rev(sort(unique(Idents(dataset.fs))))
...
for (i in 1:n.celltype.ss3){
  ...
  dataset.fs.mnn <- subset(dataset.mnn,subset = celltype == celltype.ss3 & dataset == time.ss3)
  df.cor.ss3 <- cor(t(dataset.fs.mnn@reductions$mnn@cell.embeddings),t(dataset.fs.mnn@reductions$mnn@cell.embeddings))
  cor.ss3 <- mean(apply(df.cor.ss3,2,mean))
  
  for (j in 1:n.celltype.fs){
    celltype.fs <- list.celltype.fs[j]
    dataset.fs.mnn <- subset(dataset.mnn,subset = celltype == celltype.fs & dataset == time.fs)
    
    df.cor.fs <- cor(t(dataset.fs.mnn@reductions$mnn@cell.embeddings),t(dataset.fs.mnn@reductions$mnn@cell.embeddings))
    cor.fs <- mean(apply(df.cor.fs,2,mean))
    matrix.cor[i,j] <- cor.fs # /cor.ss3
    
    df.dist.fs <- as.matrix(pdist(dataset.fs.mnn@reductions$mnn@cell.embeddings,dataset.fs.mnn@reductions$mnn@cell.embeddings))
...
Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/Comparison/",time.fs,"vs",time.ss3,"_mnn_heatmap_pearson-correlation.svg",sep="")
...
Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/Comparison/",time.fs,"vs",time.ss3,"_mnn_heatmap_euclidean-distance.svg",sep="")
      ,type="svg",width = 3+n.celltype.ss3,height = 2+n.celltype.fs,units="cm",dpi=20)
print(ggplot(data = matrix.melted, aes(x=Var1, y=Var2, fill=value)) + 
        geom_tile(color = "white")+
        scale_fill_gradient2(low = "red", high = "blue", mid = "white", 
...
celltype.i <- "PS1"
dataset.PS <- subset(dataset.fs, subset = celltype==celltype.i)
...
for (gene in genes.to.plot){
  Cairo(file=paste(directory,"/figures/2dgas-fs/Counts/ExpressionByCondition/VLNPlot-ByCondition-",celltype.i,"-",gene,".svg",sep=""),type="svg",width = 2,height = 2.5,units="cm",dpi=20)
...
dataset.t35.WNT <- subset(dataset.fs, subset = condition == "WNT" & time == "t35")
...
library(MAYA)
count_mat <- Matrix::Matrix(as.matrix(dataset.fs@assays$RNA@counts))
meta <- dataset.fs@meta.data[,c("nCount_RNA","nFeature_RNA","celltype")]
