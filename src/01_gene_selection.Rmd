...
outpath=file.path(run_outdir,file_name)
selection_short_BMP <-MI_selection %>% filter(variables %in% selected.genes_short_list_df$variables & foi %in% c(myfoi_shortlist))
MI_heatmap(selection_short_BMP,toupper(my_genes_literature),"bmp short list selection",outpath)


network_selected_variables<-unique(selected.genes_short_list_df$variables)
...
# TF miic BMP
```{r}
#  myfoi_shortlist (genes we know are intresting from literature ) + selected tfs + metadata
BMP_TF_nework_genes <-unique(c(myfoi_shortlist,selected.genes_short_list_df$variables))
#  select top mi GENES (not just tf) for each node 
if (ENABLE_MI_SELECTION) {

  mi_left_cell<-wrap_MI_compute(
    dataset =dataset.so_BMP,
    subset_vars= NULL,
    var_of_interest=BMP_TF_nework_genes,
    threads=8,
...
#  SECOND SELECTION 
# --------------------------------------------------------------
# reselect the top genes for each node of the first network
# In this second network, if a gene is not in the list of tfs, mark it as consequence (except for  metadata and the shortlist)


MI_second_selection=read.csv(file=file.path(run_outdir,"BMP_second__MI_table.csv"),sep="\t")

group_size<-5
second_selection_BMP<-MI_second_selection %>%
  group_by(foi) %>% 
  filter(!variables %in% c(mtgenes,rplgenes,rpsgenes)) %>%
  arrange(desc(value) , .by_group = TRUE) %>%
  slice_head(n = group_size) 

second_selection_genes<-unique(second_selection_BMP$variables)
network_after_second_selection<-unique(c(BMP_TF_nework_genes,second_selection_genes))
...
not_tf_not_foi<-row.names(dataset.so)[!row.names(dataset.so) %in% c(gene.list.tf,myfoi_shortlist,metadata_cols)]
...
# Generate miic outputs 
 Generate_miic_files(
     dataset.so_BMP,
     network_after_second_selection,
     metadata,
     c(myfoi_shortlist,metadata_cols),
     run_outdir,
     paste0(analysis_prefix,"_bmp_second"),
     groups=mygroups,
     assert_consequence= c(not_tf_not_foi),
     contextual=contextual
 )
...
group_size_short_list<-30
selected.genes_short_list_wnt_df<-MI_selection %>%
  filter(foi %in% myfoi_shortlist) %>% 
  group_by(foi) %>% 
  arrange(desc(value) , .by_group = TRUE) %>%
  slice_head(n = group_size_short_list) 
...
outpath=file.path(run_outdir,file_name)
selection_short <-MI_selection %>% filter(variables %in% selected.genes_short_list_wnt_df$variables & foi %in% c(myfoi_shortlist))
...
```{r}
#  myfoi_shortlist (genes we know are intresting from literature ) + selected tfs + metadata
WNT_TF_nework_genes <-unique(c(myfoi_shortlist,selected.genes_short_list_wnt_df$variables))
#  select top mi GENES (not just tf) for each node 
if (ENABLE_MI_SELECTION) {
  ...
  mi_left_cell<-wrap_MI_compute(
    dataset =dataset.so_WNT,
    subset_vars= NULL,
    var_of_interest=WNT_TF_nework_genes,
    threads=8,
    run_outdir = run_outdir,
    name="WNT_second_"
  )
...
MI_second_selection=read.csv(file=file.path(run_outdir,"WNT_second__MI_table.csv"),sep="\t")

group_size<-5
second_selection_wnt<-MI_second_selection %>%
  group_by(foi) %>% 
  filter(!variables %in% c(mtgenes,rplgenes,rpsgenes)) %>%
  arrange(desc(value) , .by_group = TRUE) %>%
  slice_head(n = group_size) 

second_selection_genes<-unique(second_selection_wnt$variables)
network_after_second_selection<-unique(c(wnt_TF_nework_genes,second_selection_genes))
...
# Generate miic outputs 
 Generate_miic_files(
     ...
     c(myfoi_shortlist,metadata_cols),
     run_outdir,
     paste0(analysis_prefix,"_wnt_second"),
     groups=mygroups,
     assert_consequence= c(not_tf_not_foi),
     contextual=contextual
 )
